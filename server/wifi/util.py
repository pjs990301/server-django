from .dataset import read_csi_csv, read_csi_from_pcap
...
def load_meta_data_fewshot(root) :
    ...
    train_x, train_y = read_csi_csv(root + '/few_shot_datasets/meta',one_file=True)
...
def load_only_testset(dir) :
    test_x, test_y = read_csi_from_pcap(dir)
    test_x = np.expand_dims(test_x, axis=1)
...
def extract_test_sample(n_way, n_support, n_query, datax, datay):
    ...
    # K = np.array(['Empty', 'Lying', 'Sitting', 'Standing', 'Walking']) # ReWis
    K = np.array(['input', 'Empty']) # ReWis
